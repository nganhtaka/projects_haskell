...
canGetSum goal liste i step nbTotal = 
  if (goal == (sum (take step (drop i liste)))) 
  then (take step (drop i liste)) 
...
calculMagic goal liste i step nbTotal = 
  if (mod goal (sum (take step (drop i liste)))) == 0 
  then [(take step (drop i liste)), (canGetSum (div goal (sum (take step (drop i liste)))) liste 0 nbTotal nbTotal)] 
  else (if ((i+step) <= nbTotal) 
...
resoudre x 1 [y] = if (div x y == y) then magicToString ([[y], [y]]) else "IMPOSSIBLE"
resoudre x n l = if (n /= (length l)) then "IMPOSSIBLE"
  magicToString (calculMagic x l 0 n n)
